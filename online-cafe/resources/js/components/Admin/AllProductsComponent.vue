<template>
    ...
    <table class="table text-center">
        ...
        <tbody>
            <tr class="clickable" v-for="product in products" :key="product.id" >
                <td style="vertical-align: middle;">{{product.name}}</td>
                <td>{{product.category_id.name}}</td>
                <td>{{formatPrice(product.price)}}</td>                
                <td style="width:25%; "><img :src="imageServerURL+product.image" style="width:25% ;border-radius: 50%;" :alt="product.name"></td>
                <td>
                    ...
                    <button class="btn btn-info mr-1" @click="visable(product.id)" v-if="product.available==0">
                    ...
                    <router-link class="btn btn-success mr-1" :to="{name: 'updateProduct', params: { productId: product.id }}">
